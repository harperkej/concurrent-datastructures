...
public class CoarseGrainedSynchronizedLinkedSet<T> implements Set<T> {
     ...
     */
    @Override
    public synchronized boolean add(Node<T> object) {
        Node[] objectSpot = findObjectSpot(object);
        Node predecessor = objectSpot[0];
        Node successor = objectSpot[1];
     ...
     */
    @Override
    public synchronized boolean remove(Node<T> object) {
        ...
        if (current != null && current.getKey() == objectKey) {
            Node successor = current.getNext();
     ...
     */
    @Override
    public synchronized boolean contains(Node<T> object) {
